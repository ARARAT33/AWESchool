...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    // Call Gemini API using modern @google/genai SDK with Web Search Grounding tool enabled!
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction,
        temperature: 0.7,
        tools: [{ googleSearch: {} }] // Enabled search grounding for full internet access!
      }
    });
